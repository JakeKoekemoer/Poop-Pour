...
export interface IClient {
     ...
     * @return OK
     */
    deleteFamily(id: string, signal?: AbortSignal): Promise<SwaggerResponse<any>>;
     ...
     * @return OK
     */
    getRoles(signal?: AbortSignal): Promise<SwaggerResponse<string[]>>;
...
export class Client extends ApiBase implements IClient {
    ...
    deleteFamily(id: string, signal?: AbortSignal): Promise<SwaggerResponse<any>> {
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.processDeleteFamily(_response);
        });
    }

    protected processDeleteFamily(response: Response): Promise<SwaggerResponse<any>> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
    ...
    protected processGetRoles(response: Response): Promise<SwaggerResponse<string[]>> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
