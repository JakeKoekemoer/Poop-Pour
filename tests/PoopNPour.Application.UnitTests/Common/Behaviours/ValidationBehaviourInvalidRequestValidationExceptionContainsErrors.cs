...
public class ValidationBehaviourInvalidRequestValidationExceptionContainsErrors
{
    private static RequestHandlerDelegate<string> Next => (ct) => Task.FromResult("result");

    [Fact]
    public async Task Handle_InvalidRequest_ValidationExceptionContainsErrors()
    {
        // Arrange
