...
public class SaveSettingsAsyncWithExtraIdentifierPassesToSettingsService
{
    ...
    public SaveSettingsAsyncWithExtraIdentifierPassesToSettingsService()
    {
    ...
    [Fact]
    public async Task SaveSettingsAsync_WithExtraIdentifier_PassesToSettingsService()
    {
        // Arrange
        ...
        var userEntity = new ApplicationUser { Id = "user-123", FirstName = "Test", LastName = "User" };
        _identityService.GetUserByIdAsync("user-123", Arg.Any<CancellationToken>())
            .Returns(userEntity);

        // Act
        await _sut.SaveSettingsAsync(settings, "extra", CancellationToken.None);

        // Assert
        _settingsService.Received(1).SaveSetting(settings, "extra");
    }
}
